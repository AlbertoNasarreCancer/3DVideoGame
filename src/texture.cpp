...
Color Image::getPixelInterpolated(float x, float y, bool repeat) {
	int ix = repeat ? (int)fmod(x,width) : (int)clamp(x,0.f,float(width-1));
	int iy = repeat ? (int)fmod(y,height) : (int)clamp(y, 0.f, float(height - 1));
	if (ix < 0) ix += width;
	if (iy < 0) iy += height;
	float fx = (x - (int)x);
	float fy = (y - (int)y);
	int ix2 = ix < (int)width - 1 ? ix + 1 : 0;
	int iy2 = iy < (int)height - 1 ? iy + 1 : 0;
